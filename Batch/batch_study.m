...
function batch_study(dataFile)
    ...
    if sum(strcmp(search_parameter(parameters, 'IndexCorrelation'), ...
            'true'))
        if search_ext_toolbox(statTool)
            batch_index_correlation(parameters, ...
                search_parameter(parameters, 'IC_Measure'), Subjects, ...
                nBands, locations, bg_color, save_check_fig, format);
        end
    end
        
    if sum(strcmp(search_parameter(parameters, 'MeasuresCorrelation'), ...
            'true'))
