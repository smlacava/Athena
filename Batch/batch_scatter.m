...
function batch_scatter(parameters)
    ...
    bands_names1 = define_bands(batch_measurePath(dataPath, measure1));
    bands_names2 = define_bands(batch_measurePath(dataPath, measure2));
    ...
    figure('Name', strcat(measure1, " ", location1, " ", ...
        bands_names1{band1}, " - ", measure2, " ", location2, " Band ", ...
        bands_names2{band2}), 'NumberTitle', 'off', 'ToolBar', 'none');
    set(gcf, 'color', [1 1 1])
    scatter(HC1, HC2, 'b')
    hold on
    scatter(PAT1, PAT2, 'r')
    legend('group 0', 'group 1')
    xlabel(strcat(measure1, " ", location1, " ", bands_names1{band1}))
    ylabel(strcat(measure2, " ", location2, " ", bands_names2{band2}))
    ...
    if str2double(search_parameter(parameters, 'save_figures')) == 1
        ...
        if strcmp(format, '.fig')
            savefig(char_check(strcat(path_check(outDir), ...
                ...
                bands_names1{band1}, '_', measure2, "_", location2, "_",...
                bands_names2{band2}, format)));
        else
            Image = getframe(gcf);
