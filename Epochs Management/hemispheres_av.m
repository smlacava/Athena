...
%
% [data_areas] = hemispheres_av(data)
%
% Input:
...
function [data_hemispheres] = hemispheres_av(data)
    [Right, Left] = asymmetry_manager(data.locations);
    data_hemispheres.measure = [];
    data_hemispheres.locations = [];
    count = 1;
    if not(isempty(Right))
        data_hemispheres.measure(:, count) = mean(data.measure(:, Right),2);
        data_hemispheres.locations = "Right";
        count = count+1;
    end
    if not(isempty(Left))
        data_hemispheres.measure(:, count) = mean(data.measure(:, Left),2);
