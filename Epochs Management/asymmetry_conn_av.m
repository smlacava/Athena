...
function [data_asymmetry] = asymmetry_conn_av(data)
    ...
    if R ~= 0 && L ~= 0
        ...
        if length(size(data.measure)) == 3
            r_meas = sum(squeeze(sum(data.measure(:, Right, Right), ...
                2)), 2)/R;
            l_meas = sum(squeeze(sum(data.measure(:, Left, Left), ...
                2)), 2)/L;
        else
            r_meas = sum(sum(data.measure(Right, Right)))/R;
            l_meas = sum(sum(data.measure(Left, Left)))/L;
        end
