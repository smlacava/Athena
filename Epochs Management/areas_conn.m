...
function [data_areas] = areas_conn(data)
    ...
    if length(size(data.measure)) == 3
        ...
        for i = 1:nLoc
            ...
            data_areas.measure(:, :, i) = sum(squeeze(sum(...
                data.measure(:, indexes{i}, indexes{i}), 3)), 2)/(n*n-n);
        end
    elseif length(size(data.measure)) == 2
        data_areas.measure = zeros(nLoc, 1);
        for i = 1:nLoc
            n = length(indexes{i});
            data_areas.measure(i) = sum(sum(...
                data.measure(indexes{i}, indexes{i}), 2), 1)/(n*n-n);
        end
    else
        data_areas.measure = zeros(size(data.measure, 1), nEpochs, nLoc);
        for i = 1:nLoc
            n = length(indexes{i});
            data_areas.measure(:, :, i) = sum(sum(...
                data.measure(:, :, indexes{i}, indexes{i}), 4), 3)/(n*n-n);
        end
    end
