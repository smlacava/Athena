%% asymmetry
% This function computes the asymmetry of a non-connectivity measure for a
...
function [data_asymmetry] = asymmetry(data)
    [Right, Left] = asymmetry_manager(data.locations);
    if not(isempty(Right) || isempty(Left))
        if length(size(data.measure)) == 3
            r_meas = mean(data.measure(:, :, Right), 3);
            l_meas = mean(data.measure(:, :, Left), 3);
        else
            r_meas = mean(data.measure(:, Right), 2);
            l_meas = mean(data.measure(:, Left), 2);
        end
