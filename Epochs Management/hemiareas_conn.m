...
%
% [data_areas] = hemiareas_conn(data)
%
% Input:
...
function [data_hemiareas] = hemiareas_conn(data)
    ...
    if length(size(data.measure)) == 4
        ...
        data_hemiareas.measure = zeros(nEpochs, nLoc, 1);
        for i = 1:nLoc
            n = length(indexes{i});
            data_hemiareas.measure(:, i) = sum(sum(...
