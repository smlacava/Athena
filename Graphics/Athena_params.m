...
function Run_Callback(hObject, eventdata, handles)
    ...
    elseif strcmp(measure, "PEntropy")
        ...
        Athena_history_update(strcat('spectral_entropy(', string(fs), ...
            ...
            strcat("'", dataPath, "'"), ",", string(tStart), ')'));
    elseif strcmp(measure, "Hurst")
        autocorrelation_measures(fs, cf, epNum, epTime, dataPath, ...
            tStart, measure)
        Athena_history_update(strcat('autocorrelation_measures(', ...
            string(fs), ',', cf_text, ',', string(epNum), ',', ...
            string(epTime), ",", strcat("'", dataPath, "'"), ",", ...
            string(tStart), ',', strcat("'", measure, "'"), ')'));
    elseif strcmpi(measure, 'Mean') || strcmpi(measure, ...
            'Median') || strcmpi(measure, 'Variance') || ...
            strcmpi(measure, 'Standard_deviation') || ...
            strcmpi(measure, 'Kurtosis') || strcmpi(measure, ...
            'Skewness')
        statistical_information(fs, cf, epNum, epTime, dataPath, ...
                tStart, measure);
        Athena_history_update(strcat('statistical_information(', ...
            string(fs), ',', cf_text, ',', string(epNum), ',', ...
            string(epTime), ",", strcat("'", dataPath, "'"), ",", ...
            string(tStart), ',', strcat("'", measure, "'"), ')'));
    elseif strcmpi(measure, "sample_entropy") || strcmpi(measure, ...
            "approximate_entropy") || strcmpi(measure, ...
            "discretized_entropy")
