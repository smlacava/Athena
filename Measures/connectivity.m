...
function []=connectivity(fs, cf, nEpochs, dt, inDir, tStart, outTypes)
    ...
    while ctrl==1
    	outTypes=[];
        sup=string(inputdlg("Insert the list of connectivity measures to save from the model separated by a comma"));
        if strcmp(sup,"exit")
        	return
        end
        sup=split(sup,",");
        for i=1:length(sup)
        	if contains(sup(i,1),["pli", "PLI", "Pli"])
            	outTypes=[outTypes, "PLI"];
            elseif contains(sup(i,1),["plv", "PLV", "Plv"])
                outTypes=[outTypes, "PLV"];
            elseif contains(sup(i,1),["aeco", "aec_o", "AECo", "AEC_o", "Aeco", "Aec_o", "AECc", "AEC_c", "aecc", "aec_c", "Aecc", "Aec_c"])
                outTypes=[outTypes, "AECo"];
            elseif contains(sup(i,1),["aec", "AEC", "Aec"])
    ...
    for i=1:length(outTypes)
        if contains(outTypes(i),["pli", "PLI", "Pli"])
        	outTypes(i)="PLI";
        elseif contains(outTypes(i),["plv", "PLV", "Plv"])
        	outTypes(i)="PLV";
        elseif contains(outTypes(i),["aeco", "aec_o", "AECo", "AEC_o", "Aeco", "Aec_o", "AECc", "AEC_c", "aecc", "aec_c", "Aecc", "Aec_c"])
        	outTypes(i)="AECo";
        elseif contains(outTypes(i),["aec", "AEC", "Aec"])
        	outTypes(i)="AEC";
        end
    end
    ...
    for c=1:length(outTypes)
        for i=1:length(cases)     
            ...
            for j=1:nBands
                for k=1:nEpochs
                    ...
                    elseif strcmp(outTypes(c),"AECo")
                        conn(j,k,:,:)=amplitude_envelope_correlation_orth(data);
                    elseif strcmp(outTypes(c),"AEC")
                        conn(j,k,:,:)=amplitude_envelope_correlation(data);
                    end
                end
            end
            ...
            save(filename,'conn');
            waitbar((i+(c-1)*length(cases))/(length(cases)*length(outTypes)),f)
        end
    end
    close(f)
end