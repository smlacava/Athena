...
function []=FOOOFer(fs, cf, nEpochs, dt, inDir, tStart, outTypes, maxPeaks)
    ...
    if strcmp(outTypes,"")
    	outTypes=[];
        sup=string(inputdlg("Insert the list of variables to save from the model separated by a comma, in order as the output directories"));
        if stcmp(sup,"exit")
            return
        end
        sup=split(sup,",");
    ...
    for i=1:length(outTypes)
        ...
        elseif contains(outTypes(i),["bg","BG", "Bg", "fit", "FIT", "Fit"])
            outTypes(i)="bg_fit";
    ...
    [x,y]=min([w(find(w>cf(end),1))-cf(end),cf(end)-w(find(w>cf(end),1)-1)]);
    superior=sup(y);
    ...
    for i=1:length(cases)
        ...
        fooofed_spectrum = zeros(nEpochs, nLoc, f_range_ind(2)-f_range_ind(1)+1);
        ...
        for k = 1:nEpochs
            for j=1:nLoc
                ...
                peak_params(k,j,1:sizePeaks)=squeeze(reshape(fooof_results.peak_params',1,sizePeaks));
                gaussian_params(k,j,1:sizePeaks)=squeeze(reshape(fooof_results.gaussian_params',1,sizePeaks));
