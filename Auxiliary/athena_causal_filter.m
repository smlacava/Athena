...
% a causal FIR bandpass filter
%
% smoothdata = athena_causal_filter(data, fs, fmin, fmax)
%
% input:
...
function smoothdata = athena_causal_filter(data, fs, fmin, fmax)
    if fmin == 0 & fmax >= fs/2
        smoothdata = data;
    else
        ...
        for i = 1:locs
            smoothdata(i, :) = filter(coef, 1, double(data(i, :)));
        end
    end
end